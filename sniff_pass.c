...
void got_packet(unsigned char *args, const struct pcap_pkthdr *header, const unsigned char *packet)
{
    ...
    if(ntohs(eth->ether_type)==0x0800)
    {
        ...
        char *data = (unsigned char *)packet + sizeof(struct sniff_ip) + sizeof(struct sniff_tcp) + sizeof(struct sniff_ethernet);
        size_data = ntohs(ip->ip_len) - (sizeof(struct sniff_ip) + sizeof(struct sniff_tcp));
        if(size_data > 0 )
        {
